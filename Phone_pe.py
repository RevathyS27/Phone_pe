...
agg_path="C:/Users/Revathy/Desktop/Youtube/.venv/Phone_pe/pulse/data/aggregated/transaction/country/india/state/"
...
#Data_Extraction_From_Github_file
User_path="C:/Users/Revathy/Desktop/Youtube/.venv/Phone_pe/pulse/data/aggregated/user/country/india/state/"
...
#Data_Extraction_From_Github_file
Map_Tnx_path="C:/Users/Revathy/Desktop/Youtube/.venv/Phone_pe/pulse/data/map/transaction/hover/country/india/state/"
...
#Data_Extraction_From_Github_file
Map_Usr_path="C:/Users/Revathy/Desktop/Youtube/.venv/Phone_pe/pulse/data/map/user/hover/country/india/state/"
...
#Data_Extraction_From_Github_file
Top_Tnx_path="C:/Users/Revathy/Desktop/Youtube/.venv/Phone_pe/pulse/data/top/transaction/country/india/state/"
...
df_Top_Dis_Transaction=pd.DataFrame(Top_Dist_Trnx)
#df_Top_Dis_Tran
#Mapping States name to geojson State name
...
#Data_Extraction_From_Github_file
Top_User_path="C:/Users/Revathy/Desktop/Youtube/.venv/Phone_pe/pulse/data/top/user/country/india/state/"
...
df_Agg_Transaction.to_sql('aggregated_transaction', engine, if_exists = 'replace', index=False,dtype={
...
def Pie_plot(select_Year, select_Quater, select_Type):
    engine = sqlalchemy.create_engine('mysql+mysqlconnector://root:root@localhost/phone_pe_project')
    if select_Type == "aggregated_transaction":
        query = f"""SELECT Transaction_type, COUNT(*) as Count
            FROM aggregated_transaction
            WHERE year={select_Year} AND quater={select_Quater}
        ...
        fig = go.Figure(data=[go.Pie(labels=Transaction_type, values=Count, marker=dict(colors=px.colors.sequential.RdBu))])
        ...
        Count = df['Count'].tolist()
        fig = go.Figure(data=[go.Pie(labels=Brand, values=Count, marker=dict(colors=px.colors.sequential.RdBu))])
        st.plotly_chart(fig)
        ...
        fig = go.Figure(data=[go.Pie(labels=District, values=Total_amount, marker=dict(colors=px.colors.sequential.RdBu))])
        st.plotly_chart(fig)

    elif select_Type == "map_user":
        query = f"""
            SELECT District, RegisteredUsers
            FROM map_user
            WHERE year={select_Year} AND quater={select_Quater}
        """
        df = pd.read_sql_query(query, engine)
        ...
        fig = go.Figure(data=[go.Pie(labels=District, values=RegisteredUsers, marker=dict(colors=px.colors.sequential.RdBu))])
        st.plotly_chart(fig)

    elif select_Type == "top_district_transaction":
        query = f"""
            SELECT District, Transaction_Count_Pin
            FROM top_district_transaction
            WHERE year={select_Year} AND quater={select_Quater}
        """
        df = pd.read_sql_query(query, engine)
        District = df['District'].tolist()
        Transaction_Count_Pin = df['Transaction_Count_Pin'].tolist()
        fig = go.Figure(data=[go.Pie(labels=District, values=Transaction_Count_Pin, marker=dict(colors=px.colors.sequential.RdBu))])
        st.plotly_chart(fig)

    elif select_Type == "top_user":
        query = f"""
            SELECT State, RegisteredUsers
            FROM top_user
            WHERE year={select_Year} AND quater={select_Quater}
        """
        df = pd.read_sql_query(query, engine)
        ...
        RegisteredUsers = df['RegisteredUsers'].tolist()
        fig = go.Figure(data=[go.Pie(labels=State, values=RegisteredUsers, marker=dict(colors=px.colors.sequential.RdBu))])
        st.plotly_chart(fig)
...
tab1, tab2, tab3 = st.tabs(["***EXPLORE_DATA***","***ANALYSIS***","***QUERIES***"])
with tab1:
    ...
    with col3:
        selected_Type = st.selectbox("Select Type", ["aggregated_transaction", "aggregated_user", "map_transaction", "map_user", "top_district_transaction", "top_user"])
    ...
    if selected_Type == "aggregated_transaction":
        color_column = "Transaction_count"
    elif selected_Type =="aggregated_user":
        color_column = "Counts"
    elif selected_Type == "map_transaction":
        color_column = "Total_amount" 
    elif selected_Type == "map_user":
        color_column = "RegisteredUsers"
    elif selected_Type == "top_district_transaction":
        color_column = "Transaction_Count_Pin"
    elif selected_Type == "top_user":
        color_column = "RegisteredUsers"
...
with tab2:
    ...
    select_Type = st.selectbox("Type", ["aggregated_transaction", "aggregated_user", "map_transaction", "map_user", "top_district_transaction", "top_user"])
...
with tab3:
    Questions=st.selectbox("Select your Question",(
                                           "States With Highest Trasaction Amount For year 2022",
                                           "Least 10 Districts based on states and amount of transaction",
                                           "Top States with highest registered users in the year 2020",
                                           "Top 10 brands based on user during the year 2021",
                                           "States With Lowest Trasaction Amount in the recent year",
                                           "Top 10 States With AppOpens in the year 2018",
                                           "Top States with highest registered users in the year 2023",
                                           "States With Highest Trasaction Amount For year 2019",
                                           "States With Lowest Trasaction Amount in the year 2021",
                                           "Top 10 States With AppOpens in the year 2020"

                                           ))
    
    if Questions=="States With Highest Trasaction Amount For year 2022":
            query1='''SELECT State, SUM(Transaction_amount) AS Total_Transaction_Amount FROM aggregated_transaction WHERE Year=2022 GROUP BY State
    ...
    elif Questions=="Least 10 Districts based on states and amount of transaction":
            query2 = '''SELECT State, District, SUM(Transaction_amount_Pin) AS Total_Transaction_Amount FROM top_district_transaction
           ...
           LIMIT 10'''
    ...
    elif Questions=="Top States with highest registered users in the year 2020":
            query3 = '''  SELECT State, RegisteredUsers FROM top_user WHERE Year = 2020 ORDER BY RegisteredUsers DESC LIMIT 10'''
            df = pd.read_sql_query(query3, engine)
            fig = px.bar(df, x='State', y='RegisteredUsers', title='Top States by Registered Users in 2020')
            st.plotly_chart(fig)

    elif Questions=="Top 10 brands based on user during the year 2021":
            query4='''SELECT Brand, COUNT(*) as Count FROM aggregated_user GROUP BY Brand ORDER BY Count DESC LIMIT 10'''
            df1 = pd.read_sql_query(query4, engine)
            fig = px.bar(df1, x='Count', y="Brand", title="Bar Plot")
            fig.update_xaxes(range=[0, 800]) 
            st.plotly_chart(fig)

    elif Questions=="States With Lowest Trasaction Amount in the recent year":
            query5='''SELECT State, SUM(Transaction_amount_Pin) AS Total_Transaction_Amount FROM top_district_transaction
                      WHERE YEAR = 2023 GROUP BY State ORDER BY Total_Transaction_Amount ASC;'''
            df1 = pd.read_sql_query(query5, engine)
            fig = px.bar(df1, x='State', y="Total_Transaction_Amount", title="Bar Plot")
            #fig.update_xaxes(range=[0, 800]
            st.plotly_chart(fig)

    elif Questions=="Top 10 States With AppOpens in the year 2018":
            query6 = '''SELECT State, COUNT(*) AS AppOpens FROM map_user WHERE YEAR= 2018 GROUP BY State
    ...
    elif Questions=="Top States with highest registered users in the year 2023":
            query7 = '''  SELECT State, RegisteredUsers FROM top_user WHERE Year = 2023 ORDER BY RegisteredUsers DESC LIMIT 10'''
            df = pd.read_sql_query(query7, engine)
            fig = px.bar(df, x='State', y='RegisteredUsers', title='Top States by Registered Users in 2023')
            st.plotly_chart(fig) 

    elif Questions=="States With Highest Trasaction Amount For year 2019":
            query8='''SELECT State, SUM(Transaction_amount) AS Total_Transaction_Amount FROM aggregated_transaction WHERE Year=2019 GROUP BY State
                     ORDER BY Total_Transaction_Amount DESC LIMIT 10'''
    ...
    elif Questions=="States With Lowest Trasaction Amount in the year 2021":
            query9='''SELECT State, SUM(Transaction_amount_Pin) AS Total_Transaction_Amount FROM top_district_transaction
                      WHERE YEAR = 2021 GROUP BY State ORDER BY Total_Transaction_Amount ASC;'''
            df1 = pd.read_sql_query(query9, engine)
            fig = px.bar(df1, x='State', y="Total_Transaction_Amount", title="Bar Plot")
            #fig.update_xaxes(range=[0, 800]
            st.plotly_chart(fig)

    elif Questions=="Top 10 States With AppOpens in the year 2020":
            query10 = '''SELECT State, COUNT(*) AS AppOpens FROM map_user WHERE YEAR= 2020 GROUP BY State
                      ORDER BY AppOpens DESC LIMIT 10'''
            ...
            plt.figure(figsize=(10, 6))
            plt.scatter(df['State'], df['AppOpens'], color='blue')
            plt.title('Top 10 States with App Opens in 2020')
            plt.xlabel('State')
            plt.ylabel('App Opens')
            plt.xticks(rotation=45)
            plt.grid(True)
